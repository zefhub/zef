...
def is_supported_value(o):
    ...
    if type(o) in {set, range, GeneratorType, list, tuple, dict, _Effect_Class, ValueType, GraphDelta, GraphSlice, Time, Image, _ErrorType}: return True
    return False
...
class ZefOp:    
    ...
    def __lshift__(self, other):
        if isinstance(other, TraversableABC):
            return ZefOp( (*self.el_ops, (RT.InIn, (other, ))) )
        if is_valid_LorO_op(other):
            return ZefOp( (*self.el_ops, *unpack_ops(RT.InIn, other.el_ops)))
        if is_tmpzefop(other):
            return ZefOp( (*self.el_ops, *unpack_tmpzefop(RT.InIn, other.el_ops))) 
    ...
    def __rshift__(self, other):
        if isinstance(other, TraversableABC):
            return ZefOp( (*self.el_ops, (RT.OutOut, (other, ))) )
        if is_valid_LorO_op(other):
            return ZefOp( (*self.el_ops, *unpack_ops(RT.OutOut, other.el_ops)))
        if is_tmpzefop(other):
            return ZefOp( (*self.el_ops, *unpack_tmpzefop(RT.OutOut, other.el_ops)))
        raise TypeError(f'Unexpected type in "ZefOp >> ..." expression. Only an RT or L[RT] makes sense here. It was called with {other} of type {type(other)}' )
    ...
    def __gt__(self, other):
        ...
        return self.lt_gt_behavior(other, RT.Out)

    def __lt__(self, other):
        if is_supported_zef_value(other): return LazyValue(other) > self
        return self.lt_gt_behavior(other, RT.In)
...
class CollectingOp:
    ...
    def __lshift__(self, other):
        return self.lshift_rshift_behavior(other, RT.InIn)

    def __rshift__(self, other):
        return self.lshift_rshift_behavior(other, RT.OutOut)
...
class Awaitable:
    ...
    def lt_gt_lshift_rshift_behavior(self, other, rt):
        ...
        if rt == RT.Out or rt == RT.In:  _terrible_global_state[(line_no, id(other))] = new_awaitable
        return new_awaitable

    def __gt__(self, other):
        return self.lt_gt_lshift_rshift_behavior(other, RT.Out)

    def __lt__(self, other):
        return self.lt_gt_lshift_rshift_behavior(other, RT.In)

    def __lshift__(self, other):
        return self.lt_gt_lshift_rshift_behavior(other, RT.InIn) 

    def __rshift__(self, other):
        return self.lt_gt_lshift_rshift_behavior(other, RT.OutOut) 
...
def rt_lt_gt_behavior(self, arg, rt):
    ...
    if isinstance(arg, ZefRef) or isinstance(arg, ZefRefs) or isinstance(arg, EZefRef) or isinstance(arg, EZefRefs):
        if rt == RT.In: return arg > ZefOp(((RT.TmpZefOp, (self,)), )) 
...
def new__lt__(self, arg):
    return rt_lt_gt_behavior(self, arg, RT.In)
...
#                                               _                                _                                       __                                                          
#                                          ___ | |  ___  _ __ ___    ___  _ __  | |_   __ _  _ __  _   _     ____  ___  / _|  ___   _ __   ___                                       
#   _____  _____  _____  _____  _____     / _ \| | / _ \| '_ ` _ \  / _ \| '_ \ | __| / _` || '__|| | | |   |_  / / _ \| |_  / _ \ | '_ \ / __|    _____  _____  _____  _____  _____ 
#  |_____||_____||_____||_____||_____|   |  __/| ||  __/| | | | | ||  __/| | | || |_ | (_| || |   | |_| |    / / |  __/|  _|| (_) || |_) |\__ \   |_____||_____||_____||_____||_____|
#                                         \___||_| \___||_| |_| |_| \___||_| |_| \__| \__,_||_|    \__, |   /___| \___||_|   \___/ | .__/ |___/                                      
#                                                                                                  |___/                           |_|                                               


# last = construct_elementary_zefop(
#     RT.Last,                                    # data type to represent this op
#     (),                                         # curried in params for the raw op
#     apply_to_lazy_value_fct = apply_to_iterable__last,    
#     get_item_fct = map_get_item_handler,    # custom behavior to deal with this_op[x]
#     repr_fct = repr__last,                            # how shoud this be displayed?
# )


# _apply_to_lazy_value_fcts={}
# _get_item_fct={}
# _repr_fct={}

# def construct_elementary_zefop(rel_type, params, apply_to_lazy_value_fct, get_item_fct=None, repr_fct=None):
#     _apply_to_lazy_value_fcts[rel_type] = apply_to_lazy_value_fct
    
#     if get_item_fct is not None:
#         _get_item_fct[rel_type] = get_item_fct
        
#     if repr_fct is not None:
#         _repr_fct[rel_type] = repr_fct
        
#     return ZefOp(((rel_type, params), ))
...
# def apply_to_iterable__last(iterable, param):
#     item = None
...
#     (),                                         # curried in params for the raw op
#     apply_to_lazy_value_fct = apply_to_iterable__last,    
#     get_item_fct = None,    # custom behavior to deal with this_op[x]
#     repr_fct = repr__last,                            # how shoud this be displayed?
# )
...
## Old iter logic ##
        # def parse_input_type(input_type: str) -> str:
        #     if input_type.__repr__() == "None": return None
        #     if "Zef" in input_type.__repr__(): return "zef"
        #     return "tools"

        # def curry_args_in_zefop(zefop, args):
        #     for arg in args: zefop = zefop[arg]
        #     return zefop

        # self.type_info = create_type_info(self)
        # chained_iter = iter(self.initial_val) if parse_input_type(self.type_info[0]) != "zef" else self.initial_val
        # # TODO construct this recusively left to write without inforcing return type. If asked using iter just return __iter__(evaluate)
        # # This should deal with all normal values and ZefRefs as well as iterables!
        # # Move it to an specific evaluation engine
        # for idx, op in enumerate(self.el_ops.el_ops):
        #     try:
        #         expected_input = parse_input_type(self.type_info[idx + 1])
        #         if len(op[1]) > 0 and isinstance(op[1][0], RelationType) and op[1][0] == RT.L: 
        #             # TODO account for nested ops to be translated
        #             current_op_translated = rt_map_to_op(RT(str(op[0]) + "L"), expected_input)(chained_iter, op[1][1][0])
        #         else:
        #             if expected_input == "zef":
        #                 current_op_translated = curry_args_in_zefop(rt_map_to_op(op[0], expected_input), [*op[1]])(chained_iter)
        #             else:
        #                 current_op_translated = rt_map_to_op(op[0], expected_input)(chained_iter, *op[1])

        #     except Exception as e:
        #         raise Exception(f"An error happened with {op} and the error is {e}")
            
        #     if not isinstance(current_op_translated, Iterable): 
        #         current_op_translated = (x for x in [current_op_translated]) # Turn single elements into a generator!
            
        #     chained_iter = iter(current_op_translated) if expected_input != "zef" else current_op_translated
        # try:
        #     return iter(chained_iter)
        # except:
        #     return chained_iter
# %%
