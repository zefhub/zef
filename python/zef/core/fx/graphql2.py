...
def graphql2_start_server_handler(eff: Effect):
    """Example
    {
        "type": FX.GraphQL22.StartServer,
...
def graphql2_stop_server_handler(eff: dict):    
    return run({**eff, 'type': FX.HTTP.StopServer})

    
def graphql2_start_playground_handler(eff: dict):
    """Example
    {
        "type": FX.GraphQL2.StartPlayground,
        "schema_dict": d,
        "g": g,
        "port": 5001, (default=5000)
        "path": "/gql", (default="/gql")
...
def graphql2_stop_playground_handler(eff: dict):    
    return run({**eff, 'type': FX.HTTP.StopServer})