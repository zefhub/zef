...
from .command_preparation_opt import dispatch_preparation
from .command_multi_rules_opt import relabel_cmds, cull_unnecessary_cmds, distinguish_cmd, resolved_variables_from_aliases
from .command_recombination_opt import recombination
from .command_ordering_opt import order_level1_commands
...
def generate_level1_commands(commands: List[PleaseCommandLevel2], gs: GraphSlice) -> Level1CommandInfo:
    ...
    print(f"Time for preparation: {dt}")
    ...
    for cmd in prepared_cmds:
        if is_a_PleaseInstantiate(cmd) and is_a_PleaseInstantiateRelation(cmd.atom):
            ...
            # TODO: Something seems missing here
            pass
        elif is_a_PleaseInstantiate(cmd) or is_a_PleaseAssign(cmd) or is_a_PleaseAlias(cmd) or is_a_PleaseTerminate(cmd) or is_a_PleaseMustLive(cmd) or is_a_PleaseBeSource(cmd) or is_a_PleaseBeTarget(cmd):
            pass
    ...
    while True:
        ...
        if debugging_active:
            ...
            print()
    dt = time.time() - start_time
    print(f"Time for simplifying cmds: {dt}")
    ...
    dt = time.time() - start_time
    print(f"Time for culling cmds: {dt}")
    ...
    return UVT_ctor_opt(Level1CommandInfo,
                        dict(gs=gs,
                             cmds=ordered_cmds,
...
def validate_and_simplify_lvl1_cmds(cmds_in):
    ...
    # print("After identify comps:", time.time() - start_time)
    ...
    # print("After id_preference:", time.time() - start_time)
    ...
    # print("After recombination:", time.time() - start_time)
