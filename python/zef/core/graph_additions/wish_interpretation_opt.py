...
from .wish_tagging_opt import Taggable, ensure_tag
...
def lvl2cmds_for_ETorAET(input: ET | AET, context: Lvl2Context):
    names = names_of_raet_opt(input)
    this_bare_raet = bare_raet_opt(input)

    d = {"atom": this_bare_raet}

    # ouids = set(names | filter[EternalUID] | collect)
    # iids = set(names | filter[Not[EternalUID]] | collect)
    ouids = {x for x in names if is_a_EternalUID(x)}
    iids = {x for x in names if not is_a_EternalUID(x)}

    if len(ouids) >= 2:
        raise Exception(f"Can't have multiple origin uids for the one object: {ouids}")
    if len(ouids) == 1:
        d["origin_uid"] = ouids[0]

    if len(iids) > 0:
        d["internal_ids"] = tuple(iids)

    return [UVT_ctor_opt(PleaseInstantiate, d)], [], context
...
def lvl2cmds_for_relation_triple(input: RelationTriple, context: Lvl2Context):
    names = names_of_raet_opt(input[1])
    bare_rt = bare_raet_opt(input[1])
