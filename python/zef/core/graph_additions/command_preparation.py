    ...
    for k, v in cmd._kwargs.items():
        ...
        if isinstance(v, set):
            ...
            if z_on_graph is None:
                ...
                for to_decide,candidates in assign_candidates.items():
                    ...
                    if len(candidates) == 0:
                        ...
                        to_assign += [(to_decide, candidates[0])]
                        existing_free.remove(candidates[0])
            ...
            for val,rel in to_assign:
                exact_ids += [origin_uid(rel)]
                cmds += [PleaseAssign(target=origin_uid(target(rel)), val=Val(val))]
...
preparation_rules = [
    (EntityValueInstance, prepare_obj_notation),
    (ObjectInstance, prepare_obj_notation),
